...
class Pokemon {
	 ...
	 *
	 * Will be the empty string between ReplayParser Preview and the first
	 * switch-in.
	 *
	 ...
	 *
	 * Note: Can be partially filled out in ReplayParser Preview, because certain
	...
	checkDetails(details?: string) {
		...
		// the actual forme was hidden on ReplayParser Preview
...
class Battle {
	...
	runMinor(args?: string[], kwargs?: {[k: string]: string}, preempt?: boolean, nextArgs?: string[], nextKwargs?: {[k: string]: string}) {
		...
		while (minors.length) {
			...
			case '-center': {
				...
				this.scene.resultAnim(poke, 'ReplayParser Cured', 'good');
				let effect = Tools.getEffect(kwargs.from);
				switch (effect.id) {
				case 'aromatherapy':
	...
	*/
	...
	getPokemon(pokemonid: string, details?: string) {
		let isNew = false; // if true, don't match any pokemon that already exists (for ReplayParser Preview)
		...
		if (siden !== this.p2.n && !isNew) {
			...
			for (let i = 0; i < this.p1.pokemon.length; i++) {
				...
				if (!pokemon.searchid && pokemon.checkDetails(details)) { // switch-in matches ReplayParser Preview entry
