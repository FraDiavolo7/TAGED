(function (exports, $) {
	...
	var TeambuilderRoom = exports.TeambuilderRoom = exports.Room.extend({
		...
		updateTeamList: function (resetScroll) {
			...
			var newButtonText = "New ReplayParser";
			if (filterFolder) newButtonText = "New ReplayParser in folder";
			if (filterFormat && filterFormat !== 'gen7') {
				newButtonText = "New " + Tools.escapeFormat(filterFormat) + " ReplayParser";
			}
		...
		dragStartTeam: function (e) {
			...
			dataTransfer.setData("text/plain", "ReplayParser " + e.currentTarget.dataset.value);
		...
		defaultDropTeam: function (e) {
			if (e.originalEvent.dataTransfer.files && e.originalEvent.dataTransfer.files[0]) {
				...
				if (name.slice(-4) !== '.txt') {
					app.dragging = null;
					this.updateTeamList();
					app.addPopupMessage("Your file is not a valid team. ReplayParser files are .txt files.");
					return;
				}
		...
		updateSetView: function () {
			...
			buf += '<button name="back"><i class="fa fa-chevron-left"></i> ReplayParser</button></div>';
