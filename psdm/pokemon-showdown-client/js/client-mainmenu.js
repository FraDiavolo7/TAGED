	...
	var MainMenuRoom = this.MainMenuRoom = this.Room.extend({
		...
		initialize: function () {
			...
			if (app.down) {
				...
				buf += '<p><label class="label">ReplayParser:</label>' + this.renderTeams() + '</p>';
		...
		updateChallenges: function (data) {
			...
			this.$('.pm-window').each(function (i, el) {
				...
				if (data.challengesFrom[userid]) {
					...
					buf += '<p><label class="label">ReplayParser:</label>' + self.renderTeams(format) + '</p>';
		...
		challenge: function (name, format, team) {
			...
			buf += '<p><label class="label">ReplayParser:</label>' + this.renderTeams(format, teamIndex) + '</p>';
	...
	var TeamPopup = this.TeamPopup = this.Popup.extend({
		initialize: function (data) {
			...
			if (format.canUseRandomTeam) {
				bufs[curBuf] += '<li><button value="-1">Random ReplayParser</button></li>';
			}
