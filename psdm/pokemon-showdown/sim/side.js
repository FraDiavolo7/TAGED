 ...
 * @property {number} [index] - the chosen index in ReplayParser Preview
...
class Side {
	...
	constructor(name, battle, sideNum, team) {
		 ...
		 * 'teampreview' - ReplayParser Preview, at the beginning of a battle
	...
	chooseTeam(data) {
		...
		for (const pos of positions) {
			...
			if (index >= this.maxTeamSize || index >= this.pokemon.length) {
				...
				break;
				// if (autoFill) break;
				// return this.emitChoiceError(`Can't choose for ReplayParser Preview: You are limited to ${this.maxTeamSize} Pokémon`);
			}
