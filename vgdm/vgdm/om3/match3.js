...
Match3 = function()
{
    ...
    var swap = function(square1, square2, checkForMatches)
    {
        if (checkForMatches) {
            ...
            ++strokeNum;
            newStroke = true;
            //console.log(square1.type.replace("images/", "").replace(".png", "") + " - " + square2.type);
        }
    ...
    var handleMatch = function(square ,match)
    {
        ...
        //console.log(match[0]);
        //console.log(match[1]);
        //console.log("LOL : " + match.length + " - " + match[0].type.replace("images/", "").replace(".png", ""));
        
        matchLength = length;
        matchColor = match[0].type.replace("images/", "").replace(".png", "");
        
        var colVal = match[0].col;
        var rowVal = match[0].row;
        var colCpt = 0;
        var rowCpt = 0;

        for(j = 0; j < length; ++j)
        {
            if (match[j].col == colVal) {
                rowCpt++;
            }
            if (match[j].row == rowVal) {
                colCpt++;
            }
        }
         
        //console.log("(" + colCpt + ", " + rowCpt + ")");
        if (colCpt > 1) {
            if (rowCpt <= 1) {
                shape = "horizontal";
            } else {
	...
	var tagedSendRequest = function (data)
	{
		console.log ( 'tagedSendRequest' );
	};
